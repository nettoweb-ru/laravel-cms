...
<div class="grid-cols-{{ $width }}">
    ...
    <x-cms::form.partials.value>
        @if ($multilang)
            ...
            @endforeach
        @else
            <input name="{{ $name }}" value="{{ $value }}" type="datetime-local" step="1" id="{{ $id }}" {{ $disabled ? 'disabled' : '' }} {!! $attributes->merge(['class' => 'input text'.($disabled ? ' disabled' : '')]) !!} autocomplete="{{ $autocomplete }}" />
        @endif
    </x-cms::form.partials.value>
